...
public class RawRecorderService extends Service {
    ...
    PendingIntent pendingIntent = PendingIntent.getActivity(
            AppGlobals.homeContext, 0, intent, 0);

    Notification.Builder notifBuilder = new Notification.Builder(
            AppGlobals.homeContext)
            .setContentTitle("PowerRecorder recording active")
            .setContentText("Recording since ").setContentIntent(pendingIntent);
    ...
    public void beginRecording() {
        ...
        AppGlobals.aRecorder = new AudioRecord(RecordingConfig.audioSource,
                AudioConfig.currentSampleRate,
                AudioConfig.currentChannelConfig, AudioConfig.currentFormat,
                AudioConfig.coreBufferSize);

        if (AppGlobals.aRecorder.getState() == AudioRecord.STATE_UNINITIALIZED) {
            Toast.makeText(AppGlobals.homeContext,
                    "Could not initialize audio record", Toast.LENGTH_LONG)
                    .show();
        } else {
            Toast.makeText(AppGlobals.homeContext,
                    "Audio successfully initialized", Toast.LENGTH_SHORT)
                    .show();
        }
        ...
        new Thread(new Runnable() {
            ...
            public void run() {
                ...
                try {
                    bufferDumpStream = AppGlobals.homeContext.openFileOutput(
                            AppGlobals.currentTempFileName1,
                ...
                do {
                    bytesRead = AppGlobals.aRecorder.read(bData, offset,
                            AudioConfig.coreBufferSize);
